...
var assetsDir = args.Length > 0 ? args[0] : @"Assets";
...
using var eyeGlow = new SKPaint { Color = new SKColor(0x8b, 0x5c, 0xf6, 0x33), IsAntialias = true };
...
using (var bmp = SKBitmap.Decode(pngPath))
{
    ...
    {
        using var resized = new SKBitmap(s, s);
        bmp.ScalePixels(resized, SKFilterQuality.High);
        using var img = SKImage.FromBitmap(resized);
        using var encData = img.Encode(SKEncodedImageFormat.Png, 100);
        encodedImages.Add(encData.ToArray());
    }
}
...
using (var writer = new BinaryWriter(fs))
{
    ...
    var entryOffset = 6 + icoSizes.Length * 16;

    for (var i = 0; i < icoSizes.Length; i++)
    {
        var s = icoSizes[i];
        var encBytes = encodedImages[i];
        ...
        writer.Write((int)encBytes.Length);
        writer.Write((int)entryOffset);
        entryOffset += encBytes.Length;
    }

    foreach (var encBytes in encodedImages)
